...
class StylingAssistantService {
  ...
  async sendMessage(
    ...
    ctx.recentQueries.push(message);
    if (ctx.recentQueries.length > 10) {
      ctx.recentQueries.shift();
    }
    this.context.set(userId, ctx);

    // Generate AI response
    const response = this.generateResponse(message, closetItems, styleProfile);
