...
class AIStyleService {
  ...
  private detectItemStyle(item: Item): Array<{ style: StyleCategory; score: number }> {
    ...
    if (name.includes('basic') || name.includes('simple') || 
        name.includes('classic') || name.includes('minimal')) {
      styles.push({ style: 'minimalist', score: 10 });
    }

    // Streetwear detection
    if (name.includes('sneaker') || name.includes('hoodie') || 
        name.includes('jogger') || name.includes('street')) {
      styles.push({ style: 'streetwear', score: 10 });
    }

    // Bohemian detection
    if (name.includes('boho') || name.includes('flowy') || 
        name.includes('maxi') || name.includes('embroidered')) {
    ...
    if (name.includes('vintage') || name.includes('retro') || 
      ...
      styles.push({ style: 'vintage', score: 10 });
    }

    // Athleisure detection
    if (category === 'activewear' || name.includes('athletic') || 
        name.includes('sport') || name.includes('yoga')) {
      styles.push({ style: 'athleisure', score: 10 });
    }

    // Formal detection
    if (category === 'formalwear' || name.includes('suit') || 
        name.includes('blazer') || name.includes('dress shirt')) {
      styles.push({ style: 'formal', score: 10 });
    }

    // Casual detection (default)
    if (styles.length === 0) {
  ...
  private calculateWardrobeStats(items: Item[]): WardrobeStats {
    ...
    return {
      ...
      leastWornCategory,
      wardrobeGaps,
      duplicateItems: 0, // Would need more logic to detect duplicates
      seasonalBalance: {
        spring: 25,
        summer: 25,
        autumn: 25,
