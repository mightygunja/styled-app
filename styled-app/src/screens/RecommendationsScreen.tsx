...
import { lookAPI, closetAPI, MOCK_USER_ID } from '../services/api';
...
export default function RecommendationsScreen() {
  ...
  const loadRecommendations = async () => {
    try {
      ...
      try {
        const favoritesResponse = await lookAPI.getFavorites(MOCK_USER_ID);
      ...
      let closetItems: any[] = [];
      try {
        const closetResponse = await closetAPI.getItems(MOCK_USER_ID);
  ...
  const handleFavorite = async (lookId: string) => {
    try {
      await lookAPI.toggleFavorite(MOCK_USER_ID, lookId);
      setFavorites(prev => {
