...
export default function WidgetScreen() {
  ...
  const loadData = async () => {
    try {
      ...
      const [settingsData, widgetsData, outfitWidgetData, statsWidgetData, analyticsData] = await Promise.all([
        widgetService.getWidgetSettings(MOCK_USER_ID),
        widgetService.getAvailableWidgets(),
        widgetService.getOutfitWidgetData(MOCK_USER_ID),
        widgetService.getStatsWidgetData(MOCK_USER_ID),
        widgetService.getWidgetAnalytics(MOCK_USER_ID),
      ]);
  ...
  const handleUpdateSettings = async (updates: Partial<WidgetSettings>) => {
    ...
    try {
      const updated = await widgetService.updateWidgetSettings(MOCK_USER_ID, updates);
      setSettings(updated);
  ...
  const handleRefreshAll = async () => {
    try {
      ...
      await widgetService.refreshAllWidgets(MOCK_USER_ID);
