...
export default function SiriShortcutsScreen() {
  ...
  const loadData = async () => {
    try {
      ...
      const [settingsData, shortcutsData, analyticsData, historyData] = await Promise.all([
        siriShortcutsService.getShortcutSettings(MOCK_USER_ID),
        siriShortcutsService.getAvailableShortcuts(),
        siriShortcutsService.getShortcutAnalytics(MOCK_USER_ID),
        siriShortcutsService.getShortcutHistory(MOCK_USER_ID),
      ]);
  ...
  const handleUpdateSettings = async (updates: Partial<ShortcutSettings>) => {
    ...
    try {
      const updated = await siriShortcutsService.updateShortcutSettings(MOCK_USER_ID, updates);
      setSettings(updated);
