...
export default function CustomBrandingScreen() {
  ...
  const loadData = async () => {
    try {
      ...
      const [brandingData, templatesData, featuresData] = await Promise.all([
        customBrandingService.getUserBranding(MOCK_USER_ID),
  ...
  const handleApplyTemplate = async (templateId: string) => {
    try {
      ...
      const updated = await customBrandingService.applyTemplate(MOCK_USER_ID, templateId);
      setBranding(updated);
  ...
  const handleExportBrandKit = async () => {
    try {
      ...
      await customBrandingService.exportBrandKit(MOCK_USER_ID);
