...
export default function PostDetailScreen() {
  ...
  const loadPost = async () => {
    try {
      ...
      const [feedPosts, postComments] = await Promise.all([
        socialFeedService.getFeed('current-user'),
  ...
  const handleLike = async () => {
    ...
    try {
      if (post.isLiked) {
        await socialFeedService.unlikePost(postId, 'current-user');
        setPost({ ...post, isLiked: false, likes: post.likes - 1 });
      } else {
        await socialFeedService.likePost(postId, 'current-user');
  ...
  const handleSave = async () => {
    ...
    try {
      if (post.isSaved) {
        await socialFeedService.unsavePost(postId, 'current-user');
        setPost({ ...post, isSaved: false, saves: post.saves - 1 });
        showToast('Removed from saved', 'success');
      } else {
        await socialFeedService.savePost(postId, 'current-user');
  ...
  const handleAddComment = async () => {
    ...
    try {
      ...
      const user = await userProfileService.getUserProfile('current-user');
  ...
  const handleDeleteComment = async (commentId: string) => {
    try {
      await socialFeedService.deleteComment(commentId, 'current-user');
