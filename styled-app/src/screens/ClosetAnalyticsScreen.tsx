...
export default function ClosetAnalyticsScreen() {
  ...
  const loadAnalytics = async () => {
    try {
      ...
      // Most/least worn (mock data for now)
      const mostWornItems = items.slice(0, 3);
      const leastWornItems = items.slice(-3);

      // Cost per wear (mock calculation)
      const costPerWear: { [key: string]: number } = {};
      items.forEach((item: any) => {
        if (item.price) {
          costPerWear[item.id] = item.price / Math.max(1, Math.floor(Math.random() * 20));
        }
      });
      ...
      <ScrollView>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            {analytics.mostWornItems.map((item: any) => (
              <View key={item.id} style={styles.itemCard}>
                ...
                <Text style={styles.itemWears}>15 wears</Text>
              </View>
            ))}
          </ScrollView>
        </View>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            {analytics.leastWornItems.map((item: any) => (
              <View key={item.id} style={styles.itemCard}>
                ...
                <Text style={styles.itemWears}>2 wears</Text>
              </View>
            ))}
          </ScrollView>
        </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.insightCard}>
            <Text style={styles.insightEmoji}>💰</Text>
            <Text style={styles.insightText}>
              Average cost per wear: ${(analytics.totalValue / (analytics.totalItems * 10)).toFixed(2)}
            </Text>
          </View>
        </View>
      </ScrollView>
