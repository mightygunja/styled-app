...
export default function PriorityBookingScreen() {
  ...
  const loadData = async () => {
    try {
      ...
      const [slotsData, statsData, benefitsData] = await Promise.all([
        ...
        priorityBookingService.getBookingStats(getCurrentUserId(), subscription.tier),
  ...
  const handleBookSlot = async (slot: BookingSlot) => {
    try {
      ...
      await priorityBookingService.bookSlot(getCurrentUserId(), slot.id, userTier);
  ...
  const handleJoinWaitlist = async (slot: BookingSlot) => {
    try {
      ...
      const booking = await priorityBookingService.joinWaitlist(getCurrentUserId(), slot.id, userTier);
      showToast(
