...
class CarbonFootprintService {
  ...
  /**
   * Deterministic per-item adjustment factor: cheaper items are more likely fast-fashion
   * (higher-turnover, lower-durability manufacturing), which is a real, if rough, proxy
   * signal available from item.price. No randomness - same item always yields the same result.
   */
  private priceAdjustmentFactor(item: Item): number {
    if (!item.price || item.price <= 0) return 1.0;
    if (item.price < 20) return 1.15;
    if (item.price < 50) return 1.0;
    if (item.price < 100) return 0.9;
    return 0.8;
  }
  ...
  private calculateMaterialsEmissions(item: Item): number {
    ...
    return baseEmission * 0.4 * this.priceAdjustmentFactor(item);
  }
  ...
  private calculateProductionEmissions(item: Item): number {
    const baseEmission = this.categoryAverages[item.category] || 20;
    return baseEmission * 0.3 * this.priceAdjustmentFactor(item);
  }
  ...
  private calculateTransportationEmissions(item: Item): number {
    const baseEmission = this.categoryAverages[item.category] || 20;
    return baseEmission * 0.15;
  }

  /**
   * Calculate use phase emissions - scales down with more wears (cost/impact amortized over use)
   */
  private calculateUseEmissions(item: Item): number {
    const baseEmission = this.categoryAverages[item.category] || 20;
    return baseEmission * 0.1;
  }
  ...
  private calculateEndOfLifeEmissions(item: Item): number {
    const baseEmission = this.categoryAverages[item.category] || 20;
    return baseEmission * 0.05;
  }

  /**
   * Get material impact breakdown - infers likely materials from real item tags/category
   * when possible, falling back to a category-appropriate default composition
   */
  private getMaterialImpact(item: Item): DetailedFootprint['materialImpact'] {
    const tagText = (item.tags || []).join(' ').toLowerCase();
    const tagMaterials = (Object.keys(this.materialEmissions) as MaterialType[]).filter(mat =>
      tagText.includes(mat)
    );

    const categoryDefaults: Record<string, MaterialType[]> = {
      tops: ['cotton', 'polyester'],
      bottoms: ['cotton', 'polyester'],
      dresses: ['polyester', 'silk'],
      outerwear: ['wool', 'polyester'],
      shoes: ['leather', 'recycled'],
      accessories: ['leather', 'recycled'],
    };

    const materials: MaterialType[] =
      tagMaterials.length > 0 ? tagMaterials : categoryDefaults[item.category] || ['cotton', 'polyester'];
  ...
  async calculateWardrobeFootprint(items: Item[]): Promise<WardrobeFootprint> {
    ...
    const footprints = await Promise.all(
      items.map(item => this.calculateItemFootprint(item))
    );
    ...
    // Timeline (last 6 months) - real, bucketed by when items were actually added
    const timeline = this.generateTimeline(footprints);
  ...
  /**
   * Generate timeline data - real cumulative footprint added per month, bucketed by
   * each item's actual createdAt/purchaseDate (last 6 months)
   */
  private generateTimeline(footprints: DetailedFootprint[]): WardrobeFootprint['timeline'] {
    const now = new Date();
